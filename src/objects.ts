...
export function isCorrect(question: Question, answer: string): boolean {
    const expectedTrim = question.expected.trim();
    const answerTrim = answer.trim();
    const expectedCap = question.expected.toLowerCase();
    const answerCap = answer.toLowerCase();

    if (answerTrim === expectedTrim || answerCap === expectedCap) {
        return true;
    } else {
        return false;
    }
 ...
 *
 * For a `short_answer_question`, any answer is valid.
 * But for a `multiple_choice_question`, the `answer` must be exactly one of the options.
 */
...
export function toShortForm(question: Question): string {
    const step1 = question.id;
    const step2 = question.name.substring(0, 10);
    return step1 + ": " + step2;
}
...
export function publishQuestion(question: Question): Question {
    const newCopy = { ...question };
    if (newCopy.published == true) {
        newCopy.published = false;
    } else {
        newCopy.published = true;
    }
    return newCopy;
}
 ...
 * The `name` should be copied over as "Copy of ORIGINAL NAME"
 * (e.g., so "Question 1" would become "Copy of Question 1").
 * The `published` field should be reset to false.
 */
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newCopy = { ...oldQuestion };
    newCopy.name = "Copy of " + oldQuestion.name;
    newCopy.id = id;
    newCopy.published = false;
    return newCopy;
}
 ...
 * Remember that the new Question MUST have its own separate copy of the `options` list,
 * rather than the same reference to the original question's list!
 * Check out the subsection about "Nested Fields" for more information.
 */
export function addOption(question: Question, newOption: string): Question {
    const newVer = { ...question, options: [...question.options, newOption] };
    return newVer;
}
 ...
 *
 * The new question will use the `body`, `type`, `options`, and `expected` of the `contentQuestion`.
 *
 * The second question will provide the `points`. The `published` status should be set to false.
 *
